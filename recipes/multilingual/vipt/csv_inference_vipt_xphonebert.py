...
def inference(model, ref_wav, text, language_id_code=None, device="cuda"):
    ...
    language_id_phoneme = language2id[language_id_code]
    text2phone_model = Text2PhonemeSequence(language=language_id_phoneme, is_cuda=False)

    input_phonemes = text2phone_model.infer_sentence(text)
    input_ids = xphonebert_tokenizer(input_phonemes, return_tensors="pt")

    token_ids = input_ids["input_ids"].to("cuda")
    attention_mask = input_ids["attention_mask"].to("cuda")

    d_vectors = embedding_to_torch(d_vector, cuda=True, device=device)

    if language_id is not None:
        language_id = id_to_torch(language_id, cuda=True, device=device)

    print("Language ID: ", language_id, language_id_code)
...
def main():
    CONFIG_PATH = "/raid/alefiury/translation/Coqui-TTS-Mod/recipes/multilingual/vipt/VIPT-CML-XPhoneBERT-July-04-2024_12+31AM-36c6b3554/config.json"
    CHECKPOINT_PATH = "/raid/alefiury/translation/Coqui-TTS-Mod/recipes/multilingual/vipt/VIPT-CML-XPhoneBERT-July-04-2024_12+31AM-36c6b3554/checkpoint_100000.pth"
    LANGUAGE_ID_PATH = "/raid/alefiury/translation/Coqui-TTS-Mod/recipes/multilingual/vipt/VIPT-CML-XPhoneBERT-July-04-2024_12+31AM-36c6b3554/language_ids.json"
    ALC_DATASET_PATH = "/raid/fred/DATASETS/dataset_alc_new_24khz"
    OUTPUT_PATH = "/raid/alefiury/translation/Coqui-TTS-Mod/recipes/multilingual/vipt/output_alc_cml_mel_xphonebert_updated_24khz"
